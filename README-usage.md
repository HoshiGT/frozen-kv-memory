...
- **`seg`**（摘要跨度，默认 1024）——也别动。256/512/1024/2048 实测
  70.0/70.7/71.1/69.9%，平坦。槽数决定能装多少，跟喂它多长的文本无关。

`draft`（默认 16）是延迟旋钮：8/16/32/128 实测 70.7/70.9/70.9/71.4%。
它是热路径的全部成本（每生成一个 token 一次前向），128 比 16 只多 0.5 点却慢 8 倍。
draft 只需要指对方向，本来就抓不住细节。
